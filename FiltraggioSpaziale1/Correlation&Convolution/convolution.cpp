/**
 * Algoritmo di CONVOLUZIONE, senza uso del filter2d()
*/

#include <iostream>
#include <opencv4/opencv2/opencv.hpp>
#include "filtered2.cpp"
...
Mat convolution(const Mat& imageInput, const Mat& mask) {
    Mat tmp;
    ...
    for(int x = 0; x < imageOutput.rows; x++) {
        for(int y = 0; y < imageOutput.cols; y++) {
            imageOutput.at<uchar>(x, y) = filtered2(pad, tmp, x, y);
        }
    }
