...
void calcGradient(const Mat& src, Mat& c00, Mat& c10, Mat& c11) {
    Mat dx, dy, dxy;
...
void calcDetAndTrack(const Mat& c00, const Mat& c11, const Mat& c10, Mat& det, Mat& track) {
    Mat diag1, diag2;
...
void harris(const Mat& src, Mat& dest) {
    Mat c00, c10, c11, blur, det,track, R;
    GaussianBlur(src, blur, Size(5,5), 0, 0);
    calcGradient(src, c00, c10, c11);
    calcDetAndTrack(c00, c11, c10, det, track);
    calcR(det, track, R);
    detectCorner(R, dest);
}
