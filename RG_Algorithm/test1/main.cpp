...
void grow(const Mat& src, const Mat& dest, Mat& mask, Point seed) {
    stack<Point> stackPointer;
    stackPointer.push(seed);
    while(!stackPointer.empty()) {  //finchè lo stack non è vuoto
        Point center = stackPointer.top();  //prelevo pixel in cima allo stack
        mask.at<uchar>(center) = 1; //imposto questo pixel ad 1 nella maschera
        stackPointer.pop(); //rimuovo pixel dallo stack
        ...
        for(int i = 0; i < 8; i++) {
            ...
            if(estimatingPoint.x < 0 || estimatingPoint.y < 0 || 
                ...
                if(dest.at<uchar>(estimatingPoint) == 0 && mask.at<uchar>(estimatingPoint) == 0
                        && delta < th) {
                            stackPointer.push(estimatingPoint);
                        }
            }
        }
    }
